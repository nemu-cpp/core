...
WebApplication::WebApplication(const Configuration& configuration, Log& log, std::shared_ptr<Observer> observer,
    Ishiko::Error& error)
    : Application(log, observer), m_routes(std::make_shared<Routes>())
{
...
WebApplication::WebApplication(const Configuration& configuration, Log& log, std::shared_ptr<Routes> routes,
    std::shared_ptr<Observer> observer, Ishiko::Error& error)
    : Application(log, observer), m_routes(routes)
{
    // TODO: construct server entirely outside of WebApplication to give maximu flexibility
    //servers().append(std::make_shared<BeastServer>(configuration.numberOfThreads(), configuration.address(),
    //    configuration.port(), *m_routes, m_views, observer, error));
